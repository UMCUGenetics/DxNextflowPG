process CONTROLFREEC_FREEC {
    ...
    input:
    tuple val(meta), path(mpileup_tumor)//, path(mpileup_tumor), path(cpn_normal), path(cpn_tumor), path(minipileup_normal), path(minipileup_tumor)
    path fasta
    path fai
    // path snp_position
    // path known_snps
    ...
    tuple val(meta), path("*_BAF.txt")          , emit: BAF, optional:true
